...
async function createSuperAdminIfNotExists() {
  try {
    ...
    if (!existingSuperAdmin) {
      ...
      console.log('ℹ️ Usuario super administrador ya existe: superadmin@sports.com');
    }
  } catch (error) {
...
async function checkTables() {
  const tables = ['institutions', 'venues', 'users', 'groups', 'athletes', 'events', 'attendance', 'publications', 'notifications'];
  ...
  try {
    ...
    const { data: institution, error } = await supabaseAdmin
      ...
      .upsert({
        ...
        description: 'Club de fútbol juvenil con sede en Madrid',
